<template>
<!-- credits: https://codepen.io/bradyhullopeter/pen/PzZJjx -->
  <div class="auth register page">
    <form>
        <h1>Create an account!</h1>
        
        <div class="form-field" v-for="(value, key) in credentials" :key="key">
          <section>
            <svg class="icon icon-person"><use xlink:href="#icon-person"></use></svg>
            <input :type="types[key] || 'text'" v-model="credentials[key]" :placeholder="key" required />
          </section>
        </div>

        <!-- BUTTONS -->
        <div class="form-buttons">
          <button type="submit" class="a-btn--filled" @click.prevent="Signup(credentials)">Sign-up!</button>
          <router-link to="/auth/login" tag="button" class="a-btn">Log in</router-link>
        </div>
...
export default {
  data () {
    return {
      // server is expecting object with following fields
      credentials: {
        first: '',
        last: '',
        username: '',
